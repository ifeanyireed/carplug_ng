...
- **WebSockets**: [Gorilla WebSocket](https://github.com/gorilla/websocket) (`github.com/gorilla/websocket v1.5.3`) for real-time duplex buyer-seller chat & typing indicators
...
- `GET /api/vehicles/:id/contact` — **Public**. Resolves verified WhatsApp contact link and phone number for a vehicle listing without scraping exposure. Checks dealership associations to provide the dealer's verified WhatsApp/phone, standardizes numbers into international E.164 digits (`234XXXXXXXXXX`), composes pre-filled greeting with vehicle specs and VIN, generates `whatsappUrl` (`https://wa.me/...`), and asynchronously logs a `whatsapp_click` inquiry lead in the `leads` table for seller telemetry.
...
### 4.18 Real-Time WebSocket Messaging Gateway
- `GET /api/ws` — **Protected** (Authenticated via query parameter `?token=<JWT>` or `Authorization: Bearer <JWT>`). Upgrades standard HTTP connections to duplex WebSockets (`101 Switching Protocols`) with custom CORS origin validation.
  - **Connection Hub (`backend/controllers/ws_hub.go`)**:
    - Centralized concurrent `WSHub` maintaining thread-safe `map[string]map[*WSClient]bool` keyed by `userID`.
    - Handles multiplexed connections per user across multiple browser tabs or devices.
    - Automated lifecycle management: 60-second ping/pong heartbeats, 10-second write wait deadline, 4096-byte maximum message buffer, and atomic cleanup on socket drop.
  - **Supported Inbound/Outbound Message Protocols**:
    - `typing`: Emitted when counterpart is typing (`{"type": "typing", "conversationId": "...", "senderName": "..."}`). Notifies counterpart UI in real-time.
    - `chat_message`: Bidirectional real-time message payload routed directly to conversation participants.
    - `new_message`: Pushed downstream to clients when a message is dispatched (`{"type": "new_message", "conversationId": "...", "message": { ... }}`).
    - `ping` / `pong`: Connection heartbeat keeping intermediate reverse proxies and cloud firewalls alive.
  - **Bidirectional Hybrid Sync (REST & WebSocket Interoperability)**:
    - `controllers.GlobalWSHub.BroadcastMessageToConversation()` is wired directly into `StartConversation` (`POST /api/conversations`) and `SendMessage` (`POST /api/conversations/:id/messages`).
    - Messages sent via standard REST endpoints are instantly broadcast via WebSockets to online participants with $< 20\text{ms}$ push latency.
    - Fallback: Client falls back seamlessly to 15s HTTP polling if the WebSocket connection drops or firewall blocks socket traffic.
...
| **42** | Reconnect Inspection Booking Flow & Escrow Funding | Done | Reconnected frontend inspection booking to backend escrow requirements: 1. Updated `DispatchButton.tsx` to call `initializePayment` with exact server tier pricing (₦25k / ₦45k / ₦75k), obtain `payment.transaction.id`, and link `escrowTransactionId` in `createInspection`. 2. Removed silent fallback redirect to `insp-001` and added user-facing error message state (`dispatchError`). 3. Cleaned up duplicate `initializePayment` definition in `api.ts`. 4. Verified with `npx eslint . && npx next build` (39/39 routes) and live end-to-end test confirming real buyer inspection creation with valid `escrowTransactionId` and dynamic tracker route navigation. |
| **43** | Real-Time WebSocket Messaging Hub (Gorilla WebSocket) | Done | Built and integrated end-to-end zero-cost bidirectional real-time messaging: 1. Installed `github.com/gorilla/websocket v1.5.3`. 2. Created thread-safe `backend/controllers/ws_hub.go` with connection registry keyed by `userID`, read/write pumps, 60s heartbeats, and JWT token authentication via query parameter `?token=` or Authorization header. 3. Started `GlobalWSHub.Run()` in `cmd/server/main.go` and registered `GET /api/ws` in `routes.go`. 4. Wired real-time broadcasts in `conversation_controller.go` (`StartConversation` and `SendMessage`) for instantaneous REST/WebSocket interoperability. 5. Upgraded `web_app/src/app/buyer/messages/page.tsx` with live socket connection lifecycle, instant push updates, typing indicators, and an animated `Live WebSocket` status badge, retaining 15s polling solely as offline fallback. 6. Verified live with automated test (`test_ws_broadcast.mjs`) confirming instant push under 20ms. Zero-cost native architecture with 0 third-party fees. |
| **45** | AI Layer: Dynamic Valuation & Fair-Price Comps Engine | Done | Built an intelligent Nigerian automotive market valuation and price comps engine without third-party API bills: 1. Modeled depreciation curves across 7 major brands (Toyota, Lexus, Honda, Mercedes-Benz, BMW, Hyundai, Kia) with annual depreciation decay rates (8.5% - 11%/yr), condition multipliers (Tokunbo 1.0, Nigerian Used 0.68, Brand New 1.55), and mileage adjustments (+/- 1.2% per 10k km variance from average). 2. Integrated MySQL peer inventory blending (65% DB active listings + 35% algorithmic model) with IQR outlier rejection. 3. Built `backend/utils/valuation.go` and `controllers/valuation_controller.go` (`GET` & `POST /api/valuation/estimate`). 4. Automated listing price classification (`marketPriceMin`, `marketPriceMax`, `priceRating`: Great Price, Fair Market, Above Market, and plain-language `priceVerdict`) in `CreateVehicle`. 5. Integrated dynamic valuation calculator into Seller Instant Cashout tool (`/seller/sell`) and Dealer Listing Composer (`/dealer/vehicles/new`). |
| **46** | AI Layer: Plain-Language Inspection Summary & Concierge Sourcing Matcher | Done | Completed AI Layer & Automation milestones: 1. Built `backend/utils/inspection_ai.go` and `GenerateInspectionAISummary` (`POST /api/inspections/ai-summary`), synthesizing 150-point checklist passes, warnings, and defects into an executive plain-language summary for non-mechanic buyers with automated Nigerian repair cost budget estimation (NGN) and recommendation tiering ("Strong Buy", "Buy with Routine Maintenance Budget", "Caution — Pre-Purchase Negotiation Advised"). 2. Upgraded Technician Report Composer (`/technician/inspections/[id]/composer`) with a 1-click "Auto-Generate AI Summary" assistant that populates score, repair budget, and verdict. 3. Built `MatchConciergeInventory` (`POST /api/leads/concierge-match`) in `lead_controller.go` ranking verified inventory candidates by budget proximity, condition match, and Tier 4/5 certification. 4. Integrated instant matching preview into Buyer Concierge (`/buyer/concierge`) with match score badges (0-99%), verified dealer inventory cards, and 1-click linking. |
| **47** | Frontend Mock Data Decoupling & Pure Live API Migration | Done | Fully decoupled frontend from all legacy dummy mock data arrays (`MOCK_VEHICLES`, `MOCK_SHOPS`, `MOCK_INSPECTIONS`, `MOCK_LEADS`, `MOCK_TECHNICIANS`, `INITIAL_SWAPS`, `INITIAL_CAMPAIGNS`, `CAR_LISTINGS`) across all 39 routes: 1. Cleaned `web_app/src/services/api.ts` removing all mock fallback traps; functions now return genuine database results or clean empty collections (`[]` / `undefined`). 2. Updated all user-facing and dashboard routes (`buyer/vehicles/[id]`, `buyer/inspections/[id]/report`, `buyer/inspections/[id]/tracker`, `buyer/inspections/book/[vehicleId]`, `buyer/inspections/book/[vehicleId]/technicians`, `buyer/map`, `buyer/compare`, `buyer/search`, `swap`, `shops/[slug]`, `dealer/dashboard`, `dealer/vehicles`, `dealer/leads`, `dealer/shop`, `seller/listings`, `technician/inspections`, `admin/leads`, `admin/swaps`, `admin/advertising`, `app/page.tsx`, and `ExploreVehiclesSection.tsx`) to initialize with live API data, pulse loading skeletons while fetching, and genuine empty states when zero records exist. 3. Preserved TypeScript type interfaces in `data/mockStore.ts` and `data/mockCars.ts`. 4. Verified with `next build` with 100% route compilation across all 39 static and dynamic routes. |
| **48** | Global NavLink with Active Route Green Highlighting | Done | Implemented reusable `NavLink` component with reactive `isActive` detection and green highlight styling across Navbar and Sidebars: 1. Created `web_app/src/components/common/NavLink.tsx` extending Next.js `Link` with pathname matching, active/inactive class resolution, and accessibility `aria-current="page"`. 2. Overhauled `web_app/src/components/layout/Navbar.tsx`: converted "Find Cars" (`/buyer/search`), "Sell Car" (`/seller/sell`), "Swap Car" (`/swap`), "More" dropdown, "My Garage" (`/buyer/garage`), and mobile drawer links to `NavLink`. When on Find Cars, it turns vivid green (`text-emerald-400 font-bold bg-emerald-500/15 border border-emerald-500/30`) while all inactive links remain white (`text-white/90`). 3. Overhauled `web_app/src/components/common/PortalShell.tsx`: converted Desktop Sidebar, Mobile Slide-over Drawer, and Quick Portal Switcher to `NavLink`. Active items feature emerald green background (`bg-emerald-50`), bold emerald text (`text-emerald-700`), emerald left border accent (`border-l-4 border-emerald-600`), and emerald icons/badges while inactive items remain neutral/white. 4. Verified with `next build` with 100% route compilation across all 39 routes. |
| **49** | Full Platform Brand Identity Transition to mycarsNg | Done | Executed immediate end-to-end platform rebrand to **mycarsNg** based on user-provided brand asset: 1. Deployed official circular gear & crown brand emblem from `web_app/public/image.png` as `/logo.png`, `/mycarsng-logo.png`, and `src/app/icon.png` for automatic Next.js favicon & touch icon rendering. 2. Created unified high-resolution brand logo component in `web_app/src/components/common/RojoLogo.tsx` and `MyCarsNgLogo.tsx` combining the gear emblem with the bold "mycars" + red "Ng" typography. 3. Rebranded root layout metadata (`layout.tsx`) with updated title, description, and apple/shortcut icons. 4. Updated top Navigation (`Navbar.tsx`), Footer with copyright and brand watermark (`Footer.tsx`), Auth Modal header (`AuthModal.tsx`), and Portal Shell (`PortalShell.tsx`). 5. Updated all platform touchpoints including swap programs, advertising network, concierge notices, dealer dispatch messaging, and direct messaging safety banners from Verza/CarPlug to mycarsNg. 6. Verified Next.js build: 40/40 routes compiled with exit code 0. |
| **50** | Pure Next.js Zero-Config Vercel Architecture & Native Security Headers | Done | Cleaned redundant deployment files and adopted native Next.js standards: 1. Migrated enterprise security headers (`X-Content-Type-Options`, `X-Frame-Options`, `X-XSS-Protection`) directly into `web_app/next.config.ts`. 2. Removed redundant `vercel.json` files and root `package.json`, keeping the frontend 100% self-contained in `web_app/`. 3. Set Vercel deployment model to native Zero-Config (Root Directory = `web_app`). 4. Verified production build: 40/40 routes compiled cleanly with exit code 0. |
| **51** | Production Audit Phase 1: Deployment Blockers Resolution | Done | Executed all Phase 1 items from `NEW_PROMPT.MD`: 1. Removed anonymous admin registration and `AdminKey` in `auth_controller.go`; public requests silently downgrade to buyer. Verified via curl: both `@carplug.ng` and hardcoded secret attempts produce `role: "buyer"`. 2. Built admin provisioning CLI at `backend/cmd/createadmin/main.go`; successfully promoted user and verified `GET /api/admin/metrics` returns HTTP 200. 3. Validated `web_app/vercel.json` with security headers. 4. Inverted `AUTO_MIGRATE` and `AUTO_SEED` defaults to `false` in `config.go`, and verified release mode fatal guard refusing startup when `GIN_MODE=release AUTO_SEED=true`. 5. Guarded `API_BASE_URL` in `web_app/src/services/api.ts` to throw error in production builds if `NEXT_PUBLIC_API_URL` is missing. 6. Verified pure live API behavior with zero mock fallbacks. Full build validation passed: `go build ./...` (0 errors), `go vet ./...` (0 errors), `npx next build` (40/40 routes). |
| **52** | Production Audit Phase 2: Pre-Deployment Hardening | Done | Executed all Phase 2 hardening items from `NEW_PROMPT.MD`: 1. Built in-memory token bucket rate limiter with auto-eviction background sweeper (`backend/middleware/rate_limit.go`) and applied to auth and upload endpoints; verified 429 throttling. 2. Added HTTP security headers middleware (`X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, HSTS). 3. Implemented global 1 MB body size limiter (`BodyLimit(1<<20)`) excluding 50MB uploads; verified 413 on oversized payloads. 4. Added `tls=preferred` to MySQL connection DSN; verified encrypted TLS handshake to Hostinger MySQL. 5. Built `RequireVerified` middleware protecting vehicle creation, leads, inspections, messaging, payments, and role upgrades; surfaced `EMAIL_NOT_VERIFIED` machine-readable code in frontend `api.ts`; verified live flow. 6. Inspected existing database rows (confirmed no `""` empty strings, only `NULL`), upgraded `EscrowTransactionID` index to `gorm:"size:64;uniqueIndex"` on MySQL, and verified live double-booking rejection via MySQL Error 1062 duplicate key constraint. Full build validation: `go build ./...` (0 errors), `go vet ./...` (0 errors), `npx next build` (40/40 routes compiled). |
